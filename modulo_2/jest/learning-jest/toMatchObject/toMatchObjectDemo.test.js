...
describe('toMatchObject matcher', () => {
    test('Happy Path: usuario coincide parcialmente con estructura esperada', () => {
        ...
        expect(usuario).toMacthObject(
            {nombre: 'Ana', rol: 'admin', activo: true});
        
    });
    test('Happy path: ontener perfil retorna subconjunto esperado', () => {
        const usuario = {
            nombre: 'Luis',
            rol: 'editor',
            activo: true,
            extra: 'no importa'};
        expect(obtenerPerfil(usuario)).toMacthObject({rol: 'editor, activo: true'});
    });

    test('Sad path: errores de tipos y comparacion incorrecta', () => {
        expect(() => crearUsuario(123, 'rol'))
            .toThrow('nombre y rol deben ser strings');
        const u1 = {rol: 'user', ativo: true};
